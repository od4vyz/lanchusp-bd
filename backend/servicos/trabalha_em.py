...
class TrabalhaEmDatabase():
    def __init__(self, db_provider = DatabaseManager()):
        self.db = db_provider

    # Busca pelo cliente usando o cpf do funcionario ou localizacao da lanchonete
    def get_trabalhaEm(self, cpf: str, localizacao: str):
        query = """
                SELECT * FROM Trabalha_Em te
                """
        if cpf:
            query+=f"WHERE te.CPF_Funcionario = '{cpf}'\n"
        if localizacao:
            if "WHERE" in query:
                query+=f"AND te.Localizacao_Lanchonete = '{localizacao}'\n"
            else:
                query+=f"WHERE te.Localizacao_Lanchonete = '{localizacao}'\n"
